...
# Truoc 2016, khong phan biet chuyen dien va truyen thong --> 1 bang Employment
# Tu 2016 moi phan ra bang chuyen dien va truyen thong
# Vi vay, tool chi tra duoc tu 2016 tro di
...
print('Phần mềm kiểm tra Visa Bulletin (2016-2024)')
...
file_tao = str(file_gio) + "_" + str(year) + ".csv"
# with open(file_tao, 'w') as csv:  
# 		csv.write('Create time','Fansical year','Month','Adjust of Status','Traditional')

with open(file_tao, 'w') as output_csv:
	header = "Created time,Fiscal year,Month,Adjust of Status,Traditional\n"
	output_csv.write(header)
    # writer = csv.writer(output_csv)
    # writer.writerow(['Create time','Fansical year','Month','Adjust of Status','Traditional'])
...
try:
	for i in fiscal_year:
		...
		record = str(gio) + ',' + str(choice) + ',' + month[18:] + ',' + eb3_unskill_chuyendien + ',' + eb3_unskill_truyenthong
